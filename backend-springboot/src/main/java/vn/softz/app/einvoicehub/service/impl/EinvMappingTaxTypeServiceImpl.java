...
/**
 * Triển khai {@link EinvMappingTaxTypeService}.
 *
 * <h3>Đặc điểm riêng của Tax Type Mapping</h3>
 * <p>Hub {@code taxTypeId} là UUID String (FK → {@code category_tax_type.id}),
 * khác với các mapping khác dùng Byte. NCC còn có thêm
 * {@code providerTaxRate} (VD: BKAV dùng "KCT" thay vì 0%).
 *
 * <h3>Mapping mẫu (BKAV)</h3>
 * <pre>
 *   Hub UUID (VAT 10%) ↔ BKAV providerTaxTypeId="1", providerTaxRate="10%"
 *   Hub UUID (Không CT) ↔ BKAV providerTaxTypeId="3", providerTaxRate="KCT"
 * </pre>
 */
@Slf4j
@Service
...
public class EinvMappingTaxTypeServiceImpl
    ...
    // ── Extra: lookup bằng VAT rate (đặc thù của Tax Type) ───────────────────

    /**
     * Tra cứu mã thuế NCC theo tỷ lệ VAT (%).
     *
     * <p>Ví dụ: VAT = 10.00 → BKAV "1", MobiFone "VAT_10".
     * Hữu ích khi POS gửi taxRate số học thay vì taxTypeId UUID.
     */
    @Override
