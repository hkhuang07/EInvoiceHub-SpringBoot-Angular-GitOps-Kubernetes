...
public class BkavDataMapper {
    ...
    private BkavInvoice.Header toBkavHeader(String lid,InvoiceData data) {
        return BkavInvoice.Header.builder()
                ...
                .payMethodID(mapToProviderInt(lid, MappingType.PAYMENT_METHOD, data.getPaymentMethodId(), 3))
    ...
    private BkavInvoice.Detail toBkavDetail(String lid,InvoiceDetailData detail) {
        ...
        return BkavInvoice.Detail.builder()
                ...
                .isIncrease(detail.getIsIncrease())
    ...
    public BkavInvoice.RequestData toBkavAdjustRequestData(String lid, InvoiceData invoiceData) {
        BkavInvoice.RequestData base = toBkavRequestData(lid, invoiceData);

        base.setOrgInvoiceIdentify(invoiceData.getOrgInvoiceIdentify());
        base.setAdjStatus(invoiceData.getIsIncrease());
        base.setReason(invoiceData.getOrgInvoiceReason());

        if (base.getInvoice() != null) {
            BkavInvoice.Header header = base.getInvoice();

            header.setOrgInvoiceIdentify(invoiceData.getOrgInvoiceIdentify());

            // BKAV mapping: True=tăng, False=giảm, null=không thay đổi số tiền
            header.setAdjStatus(invoiceData.getIsIncrease());

            // Reason
            header.setReason(invoiceData.getOrgInvoiceReason());
        }
        return base;
    }
    ...
    public static String buildOrgInvoiceIdentify(
            String orgInvoiceForm,
            String orgInvoiceSeries,
            String orgInvoiceNo) {

        if (orgInvoiceForm == null || orgInvoiceForm.isBlank()
                || orgInvoiceSeries == null || orgInvoiceSeries.isBlank()
                || orgInvoiceNo == null || orgInvoiceNo.isBlank()) {
            return null;
        }

        // Pad số hóa đơn thành 7 chữ số
        String paddedNo;
        try {
            int no = Integer.parseInt(orgInvoiceNo.trim());
            paddedNo = String.format("%07d", no);
        } catch (NumberFormatException e) {
            // Nếu không phải số thuần túy, giữ nguyên
            paddedNo = orgInvoiceNo.trim();
        }

        return String.format("[%s]_[%s]_[%s]",
                orgInvoiceForm.trim(),
                orgInvoiceSeries.trim(),
                paddedNo);
    }
}
