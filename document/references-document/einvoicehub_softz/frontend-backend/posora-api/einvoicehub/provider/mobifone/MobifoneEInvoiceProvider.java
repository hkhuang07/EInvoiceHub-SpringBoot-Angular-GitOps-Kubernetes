...
public class MobifoneEInvoiceProvider implements EInvoiceProvider {
    ...
    public InvoiceResult createInvoice(int commandType, InvoiceData invoiceData) {
        try {
            String endpoint = MobifoneApiEndpoints.SAVE_INVOICE;
            if (commandType == 101) {
                endpoint = MobifoneApiEndpoints.SAVE_INVOICE_MTT;
            }
    ...
    public InvoiceResult getInvoiceData(String lid, String invoiceGuid) {
        try {
            ...
            MobifoneInvoiceResponse response = httpClient.get(
            ...
            if (response == null || !response.isSuccess()) {
                return InvoiceResult.error(
                    response != null ? response.getMessage() : "Không nhận được phản hồi từ MobiFone"
                );
            }
            
            var data = response.getData();
            if (data == null) {
                return InvoiceResult.error("Không có dữ liệu hóa đơn");
            }
            
            Integer mobiTypeId = data.getTthdon();
            String mobiStatusName = data.getTthai();
            Integer mobiPayMethodId = null;
            
            Integer hubStatusId = reverseMapInvoiceStatus(lid, mobiStatusName);
            Integer hubTypeId = reverseMapInvoiceType(lid, mobiTypeId);
            Integer hubPayMethodId = reverseMapPaymentMethod(lid, mobiPayMethodId);
    ...
    private Integer reverseMapInvoiceStatus(String lid, String providerStatusName) {
        if (providerStatusName == null || providerStatusName.isEmpty()) {
            log.debug("[MOBI_REVERSE_MAP] INVOICE_STATUS - Provider status is null/empty");
            return null;
        }
        ...
        try {
            ...
            log.info("[MOBI_REVERSE_MAP] INVOICE_STATUS - Provider '{}' → Hub ID {}", providerStatusName, result);
            return result;
        } catch (NumberFormatException e) {
            log.warn("[MOBI_REVERSE_MAP] INVOICE_STATUS - Parse failed for '{}', fallback to provider value", internalId);
            return null;
        }
    }
    ...
    private Integer reverseMapInvoiceType(String lid, Integer providerTypeId) {
        if (providerTypeId == null) {
            log.debug("[MOBI_REVERSE_MAP] INVOICE_TYPE - Provider type is null");
            return null;
        }
        ...
        try {
            Integer result = Integer.parseInt(internalId);
            log.info("[MOBI_REVERSE_MAP] INVOICE_TYPE - Provider {} → Hub ID {}", providerTypeId, result);
            return result;
        } catch (NumberFormatException e) {
            log.warn("[MOBI_REVERSE_MAP] INVOICE_TYPE - Parse failed for '{}', fallback to provider value: {}", internalId, providerTypeId);
    ...
    private Integer reverseMapPaymentMethod(String lid, Integer providerPayMethodId) {
        if (providerPayMethodId == null) {
            log.debug("[MOBI_REVERSE_MAP] PAYMENT_METHOD - Provider payment method is null");
            return null;
        }
        ...
        try {
            Integer result = Integer.parseInt(internalId);
            log.info("[MOBI_REVERSE_MAP] PAYMENT_METHOD - Provider {} → Hub ID {}", providerPayMethodId, result);
            return result;
        } catch (NumberFormatException e) {
            log.warn("[MOBI_REVERSE_MAP] PAYMENT_METHOD - Parse failed for '{}', fallback to provider value: {}", internalId, providerPayMethodId);
