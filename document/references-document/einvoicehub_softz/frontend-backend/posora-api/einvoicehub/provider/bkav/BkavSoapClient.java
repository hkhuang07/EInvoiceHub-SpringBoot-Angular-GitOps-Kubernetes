...
public class BkavSoapClient {
    ...
    public BkavResponse executeCommand(int commandType, String commandObject) {
        try {
            ...
            String soapRequest = buildSoapEnvelope(commandType, commandObject, partnerGuid, partnerToken);
    ...
    private String buildSoapEnvelope(int commandType, String commandObject,
                                       String partnerGuid, String partnerToken) {
        try {
            String fullCommand = String.format(
                "{\"CmdType\":%d,\"CommandObject\":%s}",
                commandType,
                commandObject.startsWith("{") || commandObject.startsWith("[") ? commandObject : ("\"" + commandObject + "\"")
            );
    ...
    private BkavResponse parseSoapResponse(String soapXml, String partnerToken) {
        try {
            ...
            resultText = unescapeXml(resultText).trim();
